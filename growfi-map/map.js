...
async function bootstrap() {
  ...
  elements.sourceBadge.textContent = "GrowFi";
  elements.mapShell.classList.toggle("is-project-view", Boolean(activeProjectId));
  showLoader(activeProjectId ? "Loading Silvi Protocol project..." : "Loading Silvi Protocol map...");
  setStatus(activeProjectId ? "Loading project evidence..." : "Loading Silvi Protocol geography...");
...
function renderCurrentView({ fit = false } = {}) {
  ...
  if (activeProjectId && !activeProject) {
    ...
    elements.projectTitle.textContent = `Project ${requestedProjectId}`;
    setStatus(`Project ${requestedProjectId} did not return renderable Silvi Protocol geometry.`, true);
    updateStats(0, 0, 0, 0);
    return;
  }
  ...
  elements.projectTitle.textContent = "GrowFi Silvi Protocol";
